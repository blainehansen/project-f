...
export function parse(TOKS: string[], opts: Params): AST.Program {
	...
	function program() {
		const segments = [] as AST.CodeSegment[]
		let text = ''
		let loc = LOC()
	...
	function jsxElement(): AST.JSXElement {
		...
		const start = LOC()
		let tag = ''
		const fields = [] as AST.JSXField[]
		const references = [] as AST.JSXReference[]
		const functions = [] as AST.JSXFunction[]
		const content = [] as AST.JSXContent[]
		let hasContent = true
		let field: AST.JSXField | AST.JSXReference | AST.JSXFunction
		let insert: AST.JSXInsert | null
	...
	function jsxInsertOrComment(): AST.JSXInsert | null {
		...
		const loc = LOC()
		const flags = { hasContent: false }
		const code = embeddedCode(flags)
	...
	function jsxField(): AST.JSXField | AST.JSXReference | AST.JSXFunction {
		...
		const loc = LOC()
		const name = SPLIT(rx.identifier)
		let code: AST.EmbeddedCode
	...
	function embeddedCode(flags: ContentFlags): AST.EmbeddedCode {
		...
		const prefixLength = TOK.length
		const segments = [] as AST.CodeSegment[]
		const loc = LOC()
		let last = balancedParens(segments, '', loc, flags)
		...
		const first = segments[0] as AST.CodeText
	...
	function ERR(msg: string, loc? : { line: number, col: number, pos: number }): never {
		loc = loc || LOC()
		const frag = ` at line ${ loc.line } col ${ loc.col }: \`\`${ TOKS.join('').substr(loc.pos, 30).replace('\n', '').replace('\r', '') }''`
