...
export function compile(str: string, opts? : Options) {
	...
	const ast = parse(toks, params)
	const ast2 = transform(ast, params)
	const code = codeGen(ast2, params)
	const out = params.sourcemap === 'extract' ? sourcemap.extractMap(code, str, params)
