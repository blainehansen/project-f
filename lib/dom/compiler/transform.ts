...
const Copy = {
	...
	CodeSegments(segments: AST.CodeSegment[]) {
		return segments.map(node => node.type === AST.CodeText ? this.CodeText(node)
	...
	return {
		...
		JSXElement(node, parent) {
			if (node.tag !== 'pre') {
				...
				const content = node.content.map(c => c.type === AST.JSXText ? { ...c, text: c.text.replace(rx.trimmableWS, '') } : c)
				node = { ...node, content }
			}
...
function collapseExtraWhitespaceInTextNodes(tx: Copy): Copy {
	return {
		...
		JSXElement(node, parent) {
			if (node.tag !== 'pre') {
				const content = node.content.map(c => c.type === AST.JSXText
...
function removeEmptyTextNodes(tx: Copy): Copy {
	return {
		...tx,
		JSXElement(node, parent) {
			const content = node.content.filter(c => c.type !== AST.JSXText || c.text !== '')
			node = { ...node, content }
			return tx.JSXElement.call(this, node, parent)
		},
	}
...
function translateHTMLEntitiesToUnicodeInTextNodes(tx: Copy): Copy {
	return {
		...tx,
		JSXText(node) {
			const text = node.text.replace(rx.htmlEntity, (entity, dec, hex, named) => dec ? String.fromCharCode(parseInt(dec, 10))
	...
	return {
		...
		JSXElement(node, parent) {
			...
			if (node.kind === AST.JSXElementKind.HTML && node.content.length === 1 && content0.type === AST.JSXText) {
				const text = this.JSXText(content0)
				const textContent = { type: AST.JSXStaticField, name: 'textContent', attr: false, namespace: null, value: codeStr(text.text) }
