...
// 		if (RunningNode !== null) {
// 			let rclock = RunningClock!
// 			let sclock = node.clock
...
// 	const owner = Owner
// 	const root = fn.length === 0 ? UNOWNED : new ComputationNode(RunningClock || RootClock, null, null)
// 	let result: T = undefined!
// 	const disposer = fn.length === 0 ? null : function _dispose() {
// 		if (RunningClock !== null) {
...
// 	const node = new DataNode(RunningClock === null ? RootClock : RunningClock, value)
...
// 		const cclock = rclock === sclock ? sclock : sclock.parent!
...
// 	const data = S.data(current)
// 	const clock = RunningClock || RootClock
// 	let age = -1
...
// 	const clock = new Clock(RunningClock || RootClock)
...
// var RunningNode = null as ComputationNode | null // currently running computation
...
// 	const clocklog = rclock.preclocks === null ? rclock.preclocks = new ClockPreClockLog() : rclock.preclocks
// 	const nodelog = rnode.preclocks === null ? rnode.preclocks = new NodePreClockLog() : rnode.preclocks
...
// 	const clockcount = clocklog.clockcounts[sclock.id]
...
// 	const time = clock.parent!.time()
// 	if (clock.age < time || clock.state === STALE) {
// 		if (clock.age < time) clock.state = CURRENT
// 		if (clock.preclocks !== null)
// 			for (let i = 0; i < clock.preclocks.ids.length; i++) {
// 				const preclock = clock.preclocks.clocks[clock.preclocks.ids[i]]
...
// 	const preclocks = node.preclocks
// 	let i: number
// 	let len: number
...
// 			const upreclocks = preclocks.uclocks[i].preclocks!
// 			const uclockid = preclocks.uclockids[i]
