...
let RECONCILE_ARRAY_BATCH = 0
const RECONCILE_ARRAY_BITS = 16
const RECONCILE_ARRAY_INC = 1 << RECONCILE_ARRAY_BITS
const RECONCILE_ARRAY_MASK = RECONCILE_ARRAY_INC - 1
...
function reconcileArrays(parent: HTMLElement, ns: Node[], us: (string | Node)[]) {
	const ulen = us.length
	// n = nodes, u = updates
	// ranges defined by min and max indices
	let nmin = 0
	let nmax = ns.length - 1
	let umin = 0
	let umax = ulen - 1
	...
	const src = new Array(umax - umin + 1)
	const utext = [] as number[]
	let preserved = 0
	...
	const lcs = longestPositiveIncreasingSubsequence(src)
